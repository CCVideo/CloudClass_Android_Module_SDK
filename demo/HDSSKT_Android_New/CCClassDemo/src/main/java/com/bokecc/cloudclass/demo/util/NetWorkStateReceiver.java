...
//            if (TextUtils.isEmpty(sb.toString())) {
                if (!NetUtils.isNetworkAvailable(context)) {
                    if (!NetUtils.isMobileConnected(context) && !NetUtils.isWifiConnected(context)) {
                        ...
                        msg = "网络已经断开,请检查网络";
                        CCClassApplication.remindStrings.add(msg);
                        CCClassApplication.updataState = true;
                ...
                if (CCClassApplication.remindStrings.size() == 2) {//少于两次不需两次比对一下
                    if (CCClassApplication.remindStrings.get(0).equals(CCClassApplication.remindStrings.get(1))) {
                        CCClassApplication.updataState = false;
                    } else {
                        CCClassApplication.updataState = true;
                    }
                }
                if (CCClassApplication.remindStrings.size() > 2) {//大于两次比较后两次的
                    CCClassApplication.remindStrings.remove(0);
                    if (CCClassApplication.remindStrings.get(0).equals(CCClassApplication.remindStrings.get(1))) {
                        CCClassApplication.updataState = false;
                    } else {
                        CCClassApplication.updataState = true;
                ...
                if (CCClassApplication.updataState) {
