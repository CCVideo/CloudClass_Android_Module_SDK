...
public class QrCodeActivity extends BaseActivity implements Callback, OnClickListener {
    ...
    protected void onViewCreated() {
        CCClassApplication.sClassDirection = 0; // 每次进入扫码页面重置方向
        initView();
        initData();
    }
    ...
    private void initCamera(SurfaceHolder surfaceHolder) {
        try {
            ...
            re.printStackTrace();
//            RebootUtils.rebootWithValue(QrCodeActivity.this, SplashActivity.class,0);//重启应用
//            mDecodeManager.showPermissionDeniedDialog(this);
//            return;
        }
    ...
    public void onClick(View v) {
        if(v.getId() == R.id.id_scan_back){
            finish();
        }else if(v.getId() == R.id.id_qr_album){
            openSystemAlbum();
        }
    }
